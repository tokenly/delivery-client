...
try {
    ...
    // arguments
    $uuid          = $argv[1];
    $data          = json_decode($argv[2], 1);
    if (!$uuid) { throw new Exception("uuid is required", 1); }
    if (!$data) { throw new Exception("data is required", 1); }
} catch (Exception $e) {
    echo $e->getMessage()."\n";
    echo "Usage: ".basename(__FILE__)." <uuid> <json_data>\n";
    exit(1);
}
...
$api_result = $api->updateSourceAddress($uuid, $data);
